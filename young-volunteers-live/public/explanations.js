...
function explanationRows(a){
  ...
  (a.choices||[]).forEach(id=>{if(a.rule.allowed.includes(id)||extra.some(d=>d.id===id))return;extra.push({id,hits:r.matches,note:'This chosen sentence does not supply the complete required evidence. Compare it with both highlighted parts above.'});});
  extra.forEach(d=>{
...
function renderModel(container,s,change=null,busy=false){
  ...
  if(phase>=rows.length)container.append(el('div','TEXT BRIDGE · wanted animal work + under sixteen not allowed · WHO: C · SOFIA','model-complete'),el('p','A complete answer needs evidence for BOTH parts. Related words or one matching detail are not enough.','bridge-reason'));
}
